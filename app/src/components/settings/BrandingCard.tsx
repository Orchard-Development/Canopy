...
import { MOOD_PRESETS, type MoodPreset } from "../onboarding/presets/index";
...
async function persistBranding(config: BrandingConfig, mode?: string): Promise<void> {
  await fetch("/api/branding", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(config),
  });
  syncBootTheme(config, mode);
}

function palFromConfig(p: BrandingConfig["dark"], gradient: string) {
  return {
    primary: p.primary, accent: p.accent || p.primary,
    background: p.background, surface: p.surface, text: p.text, gradient,
  };
}
...
export function BrandingCard() {
  ...
  async function handlePresetSelect(preset: MoodPreset) {
    ...
    try {
      await persistBranding(preset.config, mode);
      setSaved(true);
  ...
  function applyAndSave(next: BrandingConfig) {
    ...
    setBranding(next);
    persistBranding(next, mode).then(() => {
      setSaved(true);
      setTimeout(() => setSaved(false), 3000);
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setBranding(config);
      await persistBranding(config, mode);
      setSaved(true);
      setTimeout(() => setSaved(false), 3000);
    } catch (err) {
  ...
  return (
    <Card sx={{ mb: 3 }}>
      <CardContent>
        ...
        <ColorPicker config={preview} mode={mode} onChange={applyAndSave} />

        <SavedThemes current={preview} onLoad={applyAndSave} />
