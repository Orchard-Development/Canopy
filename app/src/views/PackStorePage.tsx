...
export default function PackStorePage() {
  ...
  const { entitled, install, remove, refresh: refreshEntitlements } = useEntitlements();
  ...
  if (loading) {
    return (
      <PageLayout title="Pack Store" icon={<StoreIcon />}>
  ...
  return (
    <PageLayout
      title="Pack Store"
      icon={<StoreIcon />}
      badge={<Chip label={`${packs.length} packs`} size="small" />}
    >
      <Stack spacing={2}>
        <TextField
          size="small"
